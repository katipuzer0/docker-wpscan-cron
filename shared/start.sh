...
if [[ -z "$SLACK_OAUTH_TOKEN" ]]; then
    echo "Must set SLACK_OAUTH_TOKEN" 1>&2
    exit 2
fi
...
while read p; do 
  ...
  echo $TARGET
  RESULT_FILENAME=/shared/archive/wpscan.$TARGET.`date +"%Y-%m-%d"`
  wpscan --disable-tls-checks --url $TARGET --detection-mode aggressive --random-user-agent --ignore-main-redirect --force --no-banner  --api-token $WPSCAN_API_TOKEN --wp-content-dir wp-content --enumerate u,vp,vt,tt,cb,dbe -f cli-no-color --output $RESULT_FILENAME.json --format json
